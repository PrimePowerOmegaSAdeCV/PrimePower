...
class bi_website_rma(CustomerPortal):
	...
	def portal_my_rma(self, page=1, date_begin=None, date_end=None, **kw):
		...
		rma = RmaOrder.sudo().search(domain, limit=self._items_per_page, offset=pager['offset'])
		values.update({
			'date': date_begin,
			'rma': rma.sudo(),
			'page_name': 'rma',
	...
	def rma_view(self, rma_order, category='', search='', **kwargs):
		...
		return request.render('bi_website_rma.portal_my_rma_detail_view',{
			'rma_data_list': rma_order,'contacto':request.env.user.commercial_partner_id.name,
		})